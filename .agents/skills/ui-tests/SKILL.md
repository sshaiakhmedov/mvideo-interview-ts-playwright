...
11. **Organize specs by folder:** Place the `.spec.ts` file in `tests/ui/` or a project-specific subdirectory.
12. **Group related elements:** In Page Objects, if elements relate to one module/block, nest all related locators inside a single getter object. This makes reading and navigation clearer (e.g., see `MvideoHome.page.ts` for example implementation).
...
test('example', async ({ mvideoHome }) => {
  await mvideoHome.open();
  await expect(mvideoHome.page).toHaveTitle(/М.Видео/);
});
```
