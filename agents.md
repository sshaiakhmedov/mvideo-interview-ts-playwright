...
3. **Reference Benchmarks:** You **MUST** utilize `view_file` to read a neighboring, high-quality test file (e.g., `tests/ui/search.spec.ts`) to anchor your execution matrix (e.g., observing how fixtures are injected, and how `test.step()` blocks and Deep Links are constructed).
