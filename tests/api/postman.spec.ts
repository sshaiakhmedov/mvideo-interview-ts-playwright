...
test.describe('Postman Echo API Tests', () => {
  test('GET with 2 query parameters returns 200', async ({ api }) => {
    ...
    expect(response.body.args.key1).toBe('bar1');
    expect(response.body.args.key3).toBe('extraParam');
  });
  ...
  test('POST with JSON payload returns 200', async ({ api }) => {
    ...
    expect(response.body.json.name).toBe('John Doe');
    expect(response.body.json.age).toBe(30);
    expect(response.body.json.email).toBe('john.doe@example.com');
  });
});
