...
export class PostmanAPI extends BaseAPI {
  ...
  async getPostmanEcho(queryParams: PostmanEchoArgs): Promise<{ status: number; body: PostmanEchoResponse }> {
    return await this.get<PostmanEchoResponse>(API_ENDPOINTS.POSTMAN_ECHO_GET, 200, { params: queryParams });
  }

  async postPostmanEcho(payload: PostmanEchoPostPayload): Promise<{ status: number; body: PostmanEchoPostResponse }> {
